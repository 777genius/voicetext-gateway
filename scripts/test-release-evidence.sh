...
jq -S -c -n --arg source "$source_sha" --arg image "$image_digest" --arg revision "$runner_revision" '
  def bound($position;$profile;$kind;$number):
    {position:$position,profile:$profile,kind:$kind,fixture_id:"synthetic-audio",
     ...
     effect_id:("verifier-effect-"+$number),provider_request_id:("verifier-request-"+$number),
     provider_result_id:("verifier-result-"+$number)};
  {schema_version:1,source_sha:$source,image_digest:$image,campaign_id:"synthetic-verifier-campaign",
   ...
   effects:[
    bound("standalone-dg-batch";"deepgram-batch";"batch";"1"),
    bound("standalone-dg-live";"deepgram-live";"live";"2"),
    bound("mixed-dg-batch";"deepgram-batch-elevenlabs-live";"batch";"3"),
    bound("mixed-el-live";"deepgram-batch-elevenlabs-live";"live";"4"),
    bound("standalone-el-batch";"elevenlabs-batch";"batch";"5"),
    bound("standalone-el-live";"elevenlabs-live";"live";"6"),
    bound("mixed-el-batch";"elevenlabs-batch-deepgram-live";"batch";"7"),
    bound("mixed-dg-live";"elevenlabs-batch-deepgram-live";"live";"8")
...
jq -S -c -n --arg source "$source_sha" --arg image "$image_digest" --arg revision "$runner_revision" \
  ...
  def base($position;$provider;$model;$mode;$version):
    found($position) + {provider:$provider,model:$model,mode:$mode,contract_version:$version,
      language:"multi",started_at:"2026-09-02T10:00:00Z",completed_at:"2026-09-02T10:00:01Z",
      latency_ms:1000,outcome:"pass",error_classification:"none"}
      | del(.profile,.kind);
  def batch($position;$provider;$model;$version):
    base($position;$provider;$model;"batch";$version) +
      {provider_terminal:{status:"completed",provider_result_id:(found($position).provider_result_id),observed_at:"2026-09-02T10:00:01Z"}};
  def live($position;$provider;$model):
    base($position;$provider;$model;"live";2) + {accepted_frame_count:2,
      ...
      ack_first:1,ack_last:2,finalize:{status:"flushed",provider_result_id:(found($position).provider_result_id),terminal_at:"2026-09-02T10:00:01Z"}};
  {schema_version:1,source_sha:$source,image_digest:$image,campaign_id:"synthetic-verifier-campaign",
   runner:{identity:"github.com/777genius/voicetext-canary/.github/workflows/run.yml",revision:$revision},
   credential_owner:"synthetic-verifier-owner",campaign_manifest_sha256:$campaign_sha,
   fixture_manifest_sha256:$fixture_sha,result:"pass",completed_at:"2026-09-02T11:00:00Z",
   checks:[
...
reject_policy "duplicate global request ID" "$acceptance/campaign-manifest.json" '.effects[1].provider_request_id=.effects[0].provider_request_id'
reject_policy "duplicate global result ID" "$acceptance/campaign-manifest.json" '.effects[1].provider_result_id=.effects[0].provider_result_id'
reject_policy "duplicate global effect ID" "$acceptance/campaign-manifest.json" '.effects[1].effect_id=.effects[0].effect_id'
reject_policy "cross-kind duplicate ID" "$acceptance/campaign-manifest.json" '.effects[1].effect_id=.effects[0].provider_request_id'
...
reject_policy "unlinked live finalize" "$acceptance/provider-canary.json" '.checks[1].live.finalize.provider_result_id="different-result"'
reject_policy "unlinked batch terminal" "$acceptance/provider-canary.json" '.checks[0].batch.provider_terminal.provider_result_id="different-result"'
